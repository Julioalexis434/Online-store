...
const {menu, setmenu, setShowOrders } = props 
  return (
    <>
      ...
      <aside
        ...
        } bg-secundary h-full  w-24 fixed rounded-tr-xl rounded-br-xl flex flex-col justify-between transition-all z-50 lg:left-0 md:w-28 lg:w-24 border-r border-gray-700 lg:border-0`}
      >
        <div>
  ...
  <div className="lg:hidden bg-secundary w-full h-20 fixed bottom-0 rounded-tl-xl  rounded-tr-xl transition-all  border-t border-gray-600">
        <nav className="h-full text-white flex justify-between  items-center px-2 md:px-8">
          ...
          <button href="#" className="text-3xl p-4  text-primary" onClick={()=>{setShowOrders(true), setmenu(false)}}>
           <AiOutlinePieChart />
          </button>
          <button
            onClick={() => {
